...
```{r}
sumarise_exp02 %>% 
  group_by(municipio, criterio, aproach, date) %>% 
    summarise(
        total_itens = n(),
        tp_total = sum(tp), 
        fn_total = sum(fn),
...
sumarise_exp02 %>% 
  filter(criterio == 'Despesa Extra Orçamentária') %>%
  group_by(municipio, criterio, aproach, date) %>% 
    summarise(
        total_itens = n(),
        tp_total = sum(tp), 
  ...
  scale_fill_manual(values=c("#999999", "#f39422", "#537ec5", '#293a80')) +
  scale_colour_manual(values=c("black", "black", "#56B4E9", '#293a80')) +
  labs(x='Algoritmo', y="Nº de Nós Acessados", title="", color = "") + 
  guides(fill=guide_legend(title="Algoritmo")) + 
  theme(legend.position = "top")
```
