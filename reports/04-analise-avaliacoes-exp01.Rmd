...
De acordo com a Figura l é observado que os intervalos das medianas do número de nós acessados estão entre o limites da mediana de nós acessados de 14 a 19 no Epsilon-greedy e de 12 a 16 na Bfs. Ademais, por meio do intervalo da diferença entre ***Bfs - Epsilon-greedy*** é possível constatar que pode existir uma redução de até 5 nós na mediana do número de nós acessados nas avaliações com uso da Bfs ou mesmo que está diferença seja moderada ou inexistente. 
...
Conforme é mostrado na Figura h, é notado a que os intervalos de confiança das medianas do número de nós acessados encontram-se entre 15 a 19 na Dfs eentre 14 a 19 no Epilson-greedy. No mais, por meio do intervalo de confiança da diferença entre ***Epsilon-gredy - Dfs*** é possível identificar que pode existir uma diferença positiva de até 5 nós a menos nos resultados na mediana do número de nós acessados no uso do algoritmo Epsilon-greedy em comparação a Dfs. Porém pode acontecer apenas uma moderada ou inesxistente diferença entre eles ou mesmo esta diferença ser negativa em relação a escolha do Epsilon-greedy.
...
Como foi percebido anteriormente, é possível que o crawler usando o algoritmo Bfs se sobressaia em relação aos demais, foi comparado também os algoritmos Bfs e Dfs. Neste sentido, a Figura k apresenta os intervalos de confianaça para a mediana do número de acessos de nós entre os algoritmos e a diferença entre eles.

De acordo com os intervalos de confiança mostrados na Figura k os intevalos concencetram-se em 12 até 16 na Bfs e 15 até 19 na Dfs. Além disso, baseando-se na diferença ***Bfs - Dfs*** apresentando o itervalo -6 a -0.5, é constatado a existência de uma possível superioridade no desempenho do crawler focado em relação a mediana do número de nós acessados entre os critérios fiscais em até 6 nós no uso da Bfs em comparação a Dfs. No entanto, é possível que esta diferença seja discreta, sendo até mesmo imperceptível.


## Como é a estabilidade do crawler considerando cada um dos algoritmos e as combinação de portal de transparência?

Para tentar verificar a estabilidade das avaliações no uso dos diferentes algoritmos foi utilizado a variáveis f1-score e a mediana de número de nós acessados a nível de município e combinações de portais.

### F1 Score e Mediana do número de nós acessados por cada Município da Amostra 
...
metricas_result_exp01 %>% 
  ...
  ggplot(aes(y=reorder(municipio, dif))) + 
...
metricas_result_exp01 %>% 
  group_by(municipio, aproach) %>% 
  summarise(max_value = max(median_num_access_node), min_value = min(median_num_access_node), median_value=median(median_num_access_node), dif = max_value - min_value) %>% 
  ggplot(aes(y=reorder(municipio, dif))) + 
  geom_point(aes(x=min_value, color='#293a80'), size=1, shape=108) +
  geom_point(aes(x= max_value, color='#537ec5'), size=1, shape=108)  +
  geom_dumbbell(color="#e3e2e1", aes(x = min_value, xend = max_value), colour_x = "#293a80", colour_xend = "#537ec5", size=2,
  ...
  geom_point(aes(x= median_value, color='#f39422'), size=2, alpha= 0.8, shape=108) +
  scale_color_manual(name = "", values = c("#293a80", "#537ec5", "#f39422"), labels = c("Mínimo", "Máximo", "Mediana")) +
  labs(x='Mediana do número de nós acessados', y=NULL, title="") +
  facet_grid(.~ aproach)  + 
  theme(legend.position = "top")

ggsave('exp01-numero-nodes-acessados-by_municipios.png')

```

### F1 Score e Mediana do número de nós acessados por cada Combinação de portal da Amostra 
