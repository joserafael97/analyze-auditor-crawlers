...
empresas_portais %>%
    ...
    ggplot() + 
    geom_point(aes(x=num_municipios_atendidos, y=reorder(fornecedor, +num_municipios_atendidos)), color='darkblue')  
```
...
amostra %>%
    ggplot() + 
    geom_point(aes(x=`nº portais`, y=reorder(`Combinação`, +(`nº portais`))), color='darkblue')  
```
